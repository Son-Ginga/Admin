...
export default class KoenigCardBeforeAfterComponent extends Component {
    ...
    updateImageDimensions() {
        ...
        let smallestImageHeight = Math.min(
            beforeImage ? beforeImage.height : Infinity,
            afterImage ? afterImage.height : Infinity
        );

        try {
            this.imageHeight = Math.min(
                smallestImageHeight,
                parseInt(getComputedStyle(this.element).getPropertyValue('height'))
            );
        } catch (err) {
            this.imageHeight = Math.min(
                smallestImageHeight,
    ...
    constructor(owner, args) {
        ...
        if (!args.payload.cardWidth) {
            args.payload.cardWidth = 'regular';
        }
    ...
    @action
    setLayoutRegular() {
        this.args.payload.cardWidth = null;
    }
